...
//        }else{
//            button.frame =CGRectMake(0, 0, ItemWidth_right, ItemHeight_right);
//        }
        button.frame =CGRectMake(0, 0, ItemWidth, ItemHeight);
    }
...
- (void)setNewTitle:(NSString *)title viewController:(id)viewController
{
    //这里可以用一个button代替。
    UIButton *button = [UIButton buttonWithType:UIButtonTypeCustom];
    button.frame = CGRectMake(0, 0, 180, 20);
    button.tag = 888843;
//    [button setFont:[UIFont systemFontOfSize:TitleFont]];
    [button setTitleColor:TitleColor forState:UIControlStateNormal];
    [button setTitle:title forState:UIControlStateNormal];
    [button addTarget:viewController action:@selector(showMenu) forControlEvents:UIControlEventTouchUpInside];
                         ...
                         title:(NSString *)title
{
    NavBarButtonItem *buttonItem = [NavBarButtonItem defauleItemWithTarget:target
                                                                    action:action
                                                                     title:title];
    buttonItem.button.frame =CGRectMake(0, 0, 40, 30);
                         ...
                         image:(NSString *)image
{
    NavBarButtonItem *buttonItem = [NavBarButtonItem defauleItemWithTarget:target
                                                                    action:action
                                                                     image:image];
    buttonItem.button.frame =CGRectMake(0, 0, ItemWidth_right, ItemHeight_right);
    if ([image isEqualToString:@"DT_share.png"]) {
        buttonItem.button.frame =CGRectMake(0, 0, ItemWidth_right_share, ItemHeight_right_share);
    }
    self.rightBarButtonItem = [[UIBarButtonItem alloc] initWithCustomView:buttonItem.button];
...
- (void)setBackItemWithTarget:(id)target
                       action:(SEL)action bgImage:(NSString *)sender{
    
    NavBarButtonItem *buttonItem = [NavBarButtonItem defauleItemWithTarget:target action:action image:sender];
    self.leftBarButtonItem = [[UIBarButtonItem alloc] initWithCustomView:buttonItem.button];
